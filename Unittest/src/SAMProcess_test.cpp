...
{
    char* argv[] = {(char*)"./IsoLasso_Parallel",(char*)"-i",(char*)"testdata/3exon_small_40_60_depth20.sam",(char*)"-o",(char*)"output"};
    int argc = 5;
    IsoLasso::format::Args arguments=RunBinary(argv,argc);
...
TEST(SamProcess, ExonBoundary)
{
    ...
    std::vector<range_type> GT_range {std::pair<uint32_t,uint32_t>(10,59),
                                      ...
                                      std::pair<uint32_t,uint32_t>(680,680),
                                      std::pair<uint32_t,uint32_t>(681,700)};
    ...
    EXPECT_EQ(RG.CvgStats[RG.ExonBoundary[15].first][0],4);
    ...
    EXPECT_EQ(RG.CvgStats[RG.ExonBoundary[15].first][3],4);
